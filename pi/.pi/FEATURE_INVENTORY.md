...
- 남은 custom extension 엔트리포인트: **11개**
- 남은 custom slash command: **4개**
- 남은 custom tool: **7개**
- 남은 로컬 skill 후보: **41개**
...
1. **검색/코드 인덱싱 계층**
   ...
   - `code_search`, `/reindex`
2. **Git/리뷰/워크플로우 계층**
   - `/pr`, `/cr-review`, `/update-skills`
3. **UI/세션 UX 계층**
   - cursor-aware trigger, startup brand, mermaid ASCII 렌더링, custom compaction
4. **대형 skill/prompt pack 계층**
   - gstack 계열
   - visual-explainer 계열
...
3. **`/pr`**
   - commit 이후 자연스럽게 이어지는 GitHub workflow
---
...
2. **세션/UX 편의 기능**
   - startup brand, mermaid, cursor-aware triggers
3. **정책 기능**
   - `ignore-specific-context-files`, `compactions`
4. **Git/협업 자동화 기능**
   - `/pr`, `/cr-review`, `/update-skills`
