 ...
 * OpenCode의 question tool과 동일한 경험을 pi TUI에서 제공.
 */
...
const OptionSchema = Type.Object({
	...
	description: Type.Optional(Type.String({ description: "Brief explanation of this choice" })),
});

const QuestionItemSchema = Type.Object({
	question: Type.String({ description: "The full question text" }),
	header: Type.Optional(Type.String({ description: "Short label for tab bar (max 30 chars)" })),
	options: Type.Array(OptionSchema, { description: "Available choices" }),
	multiple: Type.Optional(Type.Boolean({ description: "Allow selecting multiple choices (default: false)" })),
});
...
export default function (pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptSnippet: "Ask the user questions with options to clarify requirements or get decisions",
		promptGuidelines: GUIDELINES,
		parameters: Params,

		async execute(_id, params, _signal, _onUpdate, ctx) {
			if (!ctx.hasUI) return err("Error: UI not available (non-interactive mode)");
			...
			const result = await ctx.ui.custom<QuestionResult>((tui, theme, _kb, done) => {
				...
				const answers = new Map<number, { selected: Set<string>; wasCustom: boolean }>();
				...
				const opts = () => (q() ? [...q().options, { label: "Type your own answer", description: undefined }] : []);
				const isCustomIdx = (i: number) => i === opts().length - 1;
				const answer = (qi: number) => answers.get(qi);
				const isSelected = (qi: number, label: string) => answer(qi)?.selected.has(label) ?? false;
				...
				function setAnswer(qi: number, selected: Set<string>, wasCustom: boolean) {
				...
				function submitAll(cancelled: boolean) {
						...
						answers: questions.map((question, i) => {
							const a = answer(i);
							return { question: question.question, selected: a ? [...a.selected] : [], wasCustom: a?.wasCustom ?? false };
						}),
						cancelled,
					});
				}
				...
				function handleInput(data: string) {
					...
					// Tab forward
					if (matchesKey(data, Key.tab) || (showTabs && matchesKey(data, Key.right))) {
						if (showTabs) {
					...
					// Tab backward
					if (matchesKey(data, Key.shift("tab")) || (showTabs && matchesKey(data, Key.left))) {
						if (showTabs) {
					...
					if (data === " " && q().multiple && !isCustomIdx(cursor)) return toggleOption(cursor);
				...
				function render(width: number): string[] {
					...
					if (tab === questions.length) {
						...
						for (let i = 0; i < questions.length; i++) {
							...
							if (a && a.selected.size > 0) {
								add(` ${theme.fg("muted", `${h}:`)} ${theme.fg("text", [...a.selected].join(", "))}`);
							} else {
								add(` ${theme.fg("warning", `${h}:`)} ${theme.fg("dim", "(unanswered)")}`);
							}
						}
						lines.push("");
						...
						if (question.multiple) add(theme.fg("dim", "  (select multiple)"));
						lines.push("");
						...
						for (let i = 0; i < optList.length; i++) {
							...
							let icon: string;
							if (custom) {
								icon = editing ? "✎" : customSelected ? (question.multiple ? "■" : "●") : "…";
							...
							const color = active ? "accent" : selected || customSelected ? "success" : "text";
							add(prefix + theme.fg(color, `${icon} ${opt.label}`));
							if (opt.description) add(`     ${theme.fg("muted", opt.description)}`);
							if (custom && !editing && ct) add(`     ${theme.fg("dim", `"${ct}"`)}`);
						}
					...
					if (editing) {
						...
						add(theme.fg("dim", showTabs ? " Enter submit • Tab/←→ navigate • Esc cancel" : " Enter submit • Esc cancel"));
					} else {
			...
			if (result.cancelled) {
				return {
					content: [{ type: "text", text: "User dismissed the questions. Do not continue with the dismissed topic. Adjust your approach or ask different questions if needed." }],
					details: result,
				};
			}

			const formatted = result.answers
				.map((a) => `"${a.question}" = "${a.selected.length ? a.selected.join(", ") : "Unanswered"}"`)
				.join(", ");

			return {
				content: [{ type: "text", text: `User has answered your questions: ${formatted}. Continue with the user's answers in mind.` }],
				details: result,
			};
		},
