...
function renderContextUsage(theme: any, percent: number | null, contextWindow: number): string {
	const filled = percent === null ? 0 : Math.max(0, Math.min(BAR_WIDTH, Math.round((percent / 100) * BAR_WIDTH)));
	const bar = percent === null ? "░".repeat(BAR_WIDTH) : "█".repeat(filled) + "░".repeat(BAR_WIDTH - filled);
